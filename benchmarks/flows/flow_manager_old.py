#!/usr/bin/env python3
"""
iEDA.ai Flow Manager - 多工艺库自动化物理设计流程管理器
支持 sky130 / nangate45 / asap7 / ics55
"""
...
class FlowManager:
    ...
    def __init__(self, benchmark_root: Path):
        ...
        if not self.ieda_bin.exists():
            raise RuntimeError(f"iEDA binary not found: {self.ieda_bin}")
    ...
    def generate_tcl_scripts(self, config: FlowConfig, workspace: Path):
        ...
        # 生成各阶段脚本
        self._gen_db_scripts(config, script_dir)
        self._gen_ifp_script(config, script_dir)
        self._gen_ipl_script(config, script_dir)
        self._gen_icts_script(config, script_dir)
        self._gen_irt_script(config, script_dir)
        self._gen_ito_script(config, script_dir)
        self._gen_report_scripts(config, script_dir)
    ...
    def run_flow(self, config: FlowConfig) -> Dict:
        """运行完整流程"""
        print(f"\n{'='*60}")
...
if __name__ == "__main__":
    ...
    # 测试单个设计
    config = manager.load_design_config("aes_sky130_a")
