...
ctx.tools.register(defineTool({
name: "ieda_gp_run",
description: "Execute GP actions. kind: full (one-shot complete GP for a registered design, returns run record + same-evaluator metrics vs baseline), start, advance, candidate, local_run, apply_freeze, apply_region_density, local_restart, apply_anchor. local_run restores the checkpoint, applies the scope for N iterations, then ACCEPTS the child and writes placement.def (no separate accept needed). local_restart runs one local/global candidate, accepts the local child (set force_local=1 to force it even when the instant verdict is not left_better), then starts a fresh random_init=0 global GP from that placement; it returns def HPWL for both the local restart and a same-budget raw-GP restart baseline. Every run record carries def_hpwl (DEF-level HPWL, same evaluator as verify metrics) alongside the solver-internal hpwl. timing=1 on start/full enables timing-driven GP (is_timing_effort=1 with opt_overflow_list [0.15,0.20,0.25,0.30]) and reports timing_weight_updates.",
parameters: p({ design: { type: "string" }, workdir: { type: "string", required: true }, input_def: { type: "string" }, foundry_dir: { type: "string" }, checkpoint: { type: "string" }, iterations: { type: "integer" }, seed: { type: "integer" }, random_init: { type: "integer" }, target_density: { type: "number" }, target_overflow: { type: "number" }, init_density_penalty: { type: "number" }, min_phi_coef: { type: "number" }, max_phi_coef: { type: "number" }, congestion_effort: { type: "integer" }, seed_anchor_strength: { type: "number" }, report_route_util: { type: "integer" }, timing: { type: "integer" }, congestion_model: { type: "string" }, scope: { type: "string" }, scope_seed: { type: "integer" }, scope_active_ratio: { type: "number" }, scope_active_count: { type: "integer" }, scope_instances: { type: "string" }, scope_region: { type: "string" }, halo_coeff: { type: "number" }, halo_hops: { type: "integer" }, overflow_penalty: { type: "number" }, scope_density_target: { type: "number" }, scope_density_ratio: { type: "number" }, scope_anneal_ratio: { type: "number" }, candidate_iterations: { type: "integer" }, restart_iterations: { type: "integer" }, force_local: { type: "integer" }, region: { type: "string" }, strength: { type: "number" } }),
output: out(),
execute: async (args) => {
...
throw new Error("ieda_gp_run: kind must be full|start|advance|candidate|local_run|apply_freeze|apply_region_density|local_restart|apply_anchor");
}
}));
