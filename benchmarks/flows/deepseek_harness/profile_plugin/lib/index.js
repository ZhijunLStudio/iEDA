...
ctx.tools.register(defineTool({
name: "ieda_gp_run",
description: "Execute GP actions. kind: full (one-shot complete GP for a registered design, returns run record + same-evaluator metrics vs baseline), start, advance, candidate, local_run, apply_freeze, apply_region_density, local_restart, apply_anchor. local_run restores the checkpoint, applies the scope for N iterations, then ACCEPTS the child and writes placement.def (no separate accept needed). local_restart runs one local/global candidate, accepts the local child (set force_local=1 to force it even when the instant verdict is not left_better), then starts a fresh random_init=0 global GP from that placement; it returns def HPWL for both the local restart and a same-budget raw-GP restart baseline.",
