...
if (args.kind === "metrics") { const info = readDesigns(config.iedaRoot)[args.design] || {}; mkdirSync(resolve(args.workdir), { recursive: true }); const outFile = join(resolve(args.workdir), "gp_metrics_compare.json"); const cmd = [join(config.iedaRoot, "benchmarks/flows/gp_metrics_compare.py"), "--design", args.design, "--case-root", String(info.case_root), "--macro-lef", String(info.lef || join(config.iedaRoot, "scripts/foundry/sky130/lef/sky130_fd_sc_hd_merged.lef")), "--foundry-dir", String(runtime.foundryDir(args.design)), "--congestion-model", String(args.congestion_model ?? "rudy"), ...args.timing === 0 ? ["--no-timing"] : [], "--out", outFile, ...args.raw_def ? ["raw=" + args.raw_def] : [], ...args.candidate_def ? ["candidate=" + args.candidate_def] : []]; const runResult = await runCli(config.iedaRoot, config.python, config.timeoutMs, cmd[0], cmd.slice(1)); try { const parsed = JSON.parse(readFileSync(outFile, "utf8")); return asJson({ ok: true, metrics_file: outFile, ...parsed }); } catch (error) { return asJson({ ok: false, reason: "metrics evaluator produced no valid JSON", metrics_file: outFile, stdout_tail: runResult.stdout ? String(runResult.stdout).slice(-4000) : null, stderr_tail: runResult.stderr_tail || null }); } }
