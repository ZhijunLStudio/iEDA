...
/* Agent-facing registry view: GT scalar columns (innovus_hpwl / ieda_best_hpwl)
   are stripped so the iteration loop cannot read the answer sheet; paths stay
   (innovus_def / gp_baseline_def are benchmark lanes, not scores). */
function stripGtDesigns(raw) {
	const out = {};
	for (const k of Object.keys(raw)) { const d = { ...raw[k] }; delete d.innovus_hpwl; delete d.ieda_best_hpwl; out[k] = d; }
	return out;
}
