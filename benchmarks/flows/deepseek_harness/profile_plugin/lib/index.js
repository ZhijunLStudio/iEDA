...
/* Agent-facing registry view: everything Innovus (paths, scores, GT seeds)
   is stripped so the iteration loop cannot see the external GT at all;
   gp_baseline_def (iEDA self-history) stays as the candidate's
   own-toolchain baseline. */
function stripGtDesigns(raw) {
	const out = {};
	for (const k of Object.keys(raw)) { const d = { ...raw[k] }; delete d.innovus_hpwl; delete d.ieda_best_hpwl; delete d.innovus_def; delete d.alt_input_defs; out[k] = d; }
	return out;
}
...
if (k === "full" || k === "start" || k === "local_run" || k === "candidate" || k === "local_congestion" || k === "apply_region_density" || k === "apply_freeze" || k === "local_restart" || k === "apply_anchor") archiveRunArtifacts(args.workdir);
...
if (k === "full") { const design = args.design; if (!design) return asJson({ ok: false, reason: "full requires design" }); const info = readDesigns(config.iedaRoot)[design] || {}; const runResult = await agent(args, ["start", "--iterations", String(args.iterations ?? 200), "--seed", String(args.seed ?? 1000), "--random-init", String(args.random_init ?? 1), "--target-density", String(args.target_density ?? -1), "--target-overflow", String(args.target_overflow ?? -1), "--congestion-effort", String(args.congestion_effort ?? -1), "--timing", String(args.timing ?? 0), "--bin-cnt", String(args.bin_cnt ?? -1), "--report-route-util", String(args.report_route_util ?? 1)]); if (!runResult.ok) return asJson(runResult); mkdirSync(resolve(args.workdir), { recursive: true }); const outFile = join(resolve(args.workdir), "gp_full_metrics.json"); const candidateDef = join(resolve(args.workdir), "placement.def"); /* The baseline is the design's own incoming placement (gp_baseline_def is iEDA self-history when present, input_def otherwise) - no external GT is required. Innovus is intentionally absent from the agent loop; post-hoc comparison runs outside the plugin. */ const baselineDef = info.gp_baseline_def && existsSync(info.gp_baseline_def) ? info.gp_baseline_def : info.input_def; if (!baselineDef) return asJson({ ok: false, reason: "no baseline DEF: registry gp_baseline_def missing on disk and design has no input_def" }); const cmd = [join(config.iedaRoot, "benchmarks/flows/gp_metrics_compare.py"), "--design", design, "--case-root", String(info.case_root), "--macro-lef", String(info.lef || join(config.iedaRoot, "scripts/foundry/sky130/lef/sky130_fd_sc_hd_merged.lef")), "--foundry-dir", String(runtime.foundryDir(design)), "--congestion-model", String(args.congestion_model ?? "rudy"), ...args.timing === 0 ? ["--no-timing"] : [], "--out", outFile, "raw=" + baselineDef, "candidate=" + candidateDef]; const metricRun = await runCli(config.iedaRoot, config.python, config.timeoutMs, cmd[0], cmd.slice(1)); try { const metrics = JSON.parse(readFileSync(outFile, "utf8")); return asJson({ ok: true, run: runResult, metrics }); } catch (_) { return asJson({ ok: false, reason: "full run finished but metrics parse failed", run: runResult, stdout_tail: metricRun.stdout ? String(metricRun.stdout).slice(-4000) : null }); } }
...
if (k === "apply_anchor") { const strength = Number(args.strength ?? 0.5); if (!(strength > 0 && strength <= 1)) return asJson({ ok: false, reason: "apply_anchor requires strength in (0,1]" }); if (!args.scope_region && !args.scope_instances) return asJson({ ok: false, reason: "apply_anchor requires scope_region 'llx lly urx ury' or scope_instances 'name[,...]'" }); return agent(args, ["apply_anchor", "--iterations", String(args.iterations ?? 20), ...scopeArgs({ ...args, scope_anchor_strength: strength }), ...args.checkpoint ? ["--checkpoint", args.checkpoint] : []]); }
