...
const agent = async (args) => asJson(await runtime.agent(args.design, args.workdir, args));
const restore = async (args) => asJson(await runCli(config.iedaRoot, config.python, config.timeoutMs, join(config.iedaRoot, "benchmarks/flows/gp_agent.py"), ["--workdir", resolve(args.workdir), "restore", ...args.checkpoint ? ["--checkpoint", args.checkpoint] : []]));
const scopeArgs = (args) => ["--scope", args.scope ?? "region", "--scope-active-ratio", String(args.scope_active_ratio ?? 0.2), "--scope-active-count", String(args.scope_active_count ?? 100), "--scope-instances", args.scope_instances ?? "", "--scope-region", args.scope_region ?? "", "--halo-coeff", String(args.halo_coeff ?? 0.5), "--halo-hops", String(args.halo_hops ?? 1), "--scope-density-target", String(args.scope_density_target ?? 1), "--scope-density-ratio", String(args.scope_density_ratio ?? 0), "--overflow-penalty", String(args.overflow_penalty ?? 0)];
...
description: "Propose GP actions without changing state. kind: regions (iEDA proposes concrete regions from priority and size bounds), region_density (suggest a density target for a region), freeze (cell set that would be frozen in a region). prediction_status is unavailable until a predictor exists.",
