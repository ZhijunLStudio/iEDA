...
export function apply(ctx: Context, config: Config): void {
    ...
    execute: async () => {
      ...
      for (const [design, record] of Object.entries(designs)) {
        ...
        out[design] = {
          ...
          innovus_hpwl: hpwl,
          ieda_best_hpwl: best,
          ieda_improvement_pct: hpwl > 0 ? Math.round((1 - best / hpwl) * 100_000) / 1000 : 0,
  ...
  ctx.tools.register(defineTool({
    ...
    parameters: {
      design: { type: 'string', required: true },
      def_path: { type: 'string', required: true, description: 'Absolute path of the placement DEF.' },
    },
    output: {
      schema: commonResultSchema(),
      render: (_args, value) => jsonContent(value as Record<string, unknown>),
    ...
    execute: async args => {
      const designs = readDesigns(config.iedaRoot)
      const record = designs[args.design]
      ...
      const result = await runCli(config.iedaRoot, config.python, config.timeoutMs,
        join(config.iedaRoot, 'benchmarks/flows/def_hpwl_eval.py'), [lef, resolve(args.def_path)])
      const match = /HPWL=(\d+)/.exec(String(result.stdout ?? ''))
      if (!match) return asJson({ ok: false, error: 'could not parse def_hpwl_eval HPWL', result })
      ...
      return asJson({
        ...
        ratio: Math.round((hpwl / innovus) * 1_000_000) / 1_000_000,
        improvement_pct_vs_innovus: Math.round((1 - hpwl / innovus) * 100_000) / 1000,
      })
    },
  }))
  ...
  ctx.tools.register(defineTool({
    ...
    parameters: {
      design: { type: 'string', required: true },
      result_root: { type: 'string', required: true, description: 'Absolute output directory for run_ipl_full_compare.py.' },
      timing: { type: 'boolean', description: 'true enables timing-driven GP.' },
    },
    output: {
      schema: commonResultSchema(),
      render: (_args, value) => jsonContent(value as Record<string, unknown>),
