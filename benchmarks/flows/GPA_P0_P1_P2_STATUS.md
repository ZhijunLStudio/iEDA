...
4. s1238 路由对照（同一 iRT 脚本）：
   ...
   额外控制实验：
   - iEDA congestion-effort placement 路由 `residual_drc=3058`
     （初始 GR overflow 从 20 降到 6）；
   - congestion-effort + top_routing_layer=met5 路由
     `residual_drc=2615`（初始 GR overflow 降到 2）；
   - 相比 WL full 的 3471，congestion + met5 已降低 24.6% DRC，但仍未 clean；
