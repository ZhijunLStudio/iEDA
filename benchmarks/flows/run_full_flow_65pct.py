#!/usr/bin/env python3
"""
基于成功的 test_single_65pct.py，扩展为完整的 place→route→gds 流程
"""
...
def get_pdk_config(design_name):
    """获取 PDK 配置（从 test_single_65pct.py 复制）"""
...
def run_full_flow(design_name):
    """运行单个设计的完整流程"""
    print(f"\n{'='*80}")
    print(f"Running full flow: {design_name}")
    print(f"{'='*80}\n")

    workspace = REPO_ROOT / f"benchmarks/results/aes13_65pct/{design_name}/workspace"
    pdk_cfg = get_pdk_config(design_name)
...
if __name__ == "__main__":
    ...
    for design in designs:
        if run_full_flow(design):
            success_count += 1

    print(f"\n{'='*80}")
    print(f"Final result: {success_count}/{len(designs)} designs completed")
    print(f"{'='*80}\n")
