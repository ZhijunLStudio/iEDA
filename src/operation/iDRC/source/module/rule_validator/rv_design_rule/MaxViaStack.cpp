...
{
  const MaxViaStackRule& max_via_stack_rule = DRCDM.getDatabase().get_max_via_stack_rule();
  ...
  {
    const std::vector<int32_t>* cut_layer_idx_list = findAdjacentCutLayersByRouting(max_via_stack_rule.bottom_routing_layer_idx);
    if (cut_layer_idx_list == nullptr || cut_layer_idx_list->empty()) {
      return;
    }
  ...
  {
    const std::vector<int32_t>* cut_layer_idx_list = findAdjacentCutLayersByRouting(max_via_stack_rule.top_routing_layer_idx);
    if (cut_layer_idx_list == nullptr || cut_layer_idx_list->empty()) {
      return;
    }
  ...
  for (auto& [cut_layer_idx, net_rect_map] : cut_net_rect_map) {
    ...
    for (std::pair<int32_t, PlanarRect>& net_stack_rect_pair : net_stack_rect_list) {
      ...
      for (auto& [curr_cut_layer_idx, net_stack_rect_map] : layer_net_stack_rect_map) {
        ...
        if (!getMinAdjacentRoutingLayerByCut(curr_cut_layer_idx, routing_layer_idx)) {
          continue;
        }
