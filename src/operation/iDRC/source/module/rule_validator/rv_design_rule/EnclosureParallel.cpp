...
void RuleValidator::verifyEnclosureParallel(RVCluster& rv_cluster)
{
  ...
  for (const auto& [cut_layer_idx, cut_layer_data] : layer_data) {
    ...
    const auto routing_layer_iter = cut_to_adjacent_routing_map.find(cut_layer_idx);
    if (routing_layer_iter == cut_to_adjacent_routing_map.end()) {
      continue;
    }
    const std::vector<int32_t>& routing_layer_idx_list = routing_layer_iter->second;
