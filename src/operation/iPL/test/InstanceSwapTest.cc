...
auto verifyNoOp(const InstanceSwapResult& result, const char* stage_name) -> bool
{
  bool ok = true;
  ok &= require(result.isSuccessful(), "instance swap no-op must be a successful result");
  ok &= require(result.outcome == InstanceSwapOutcome::kNoOp, "instance swap must expose an explicit no-op outcome");
  ok &= require(result.accepted_count == 0, "instance swap no-op must accept no candidates");
  ok &= require(result.changed_count == 0, "instance swap no-op must change no coordinates");
  ok &= require(result.hpwl_before == result.hpwl_after, "instance swap no-op must preserve HPWL");
  ok &= require(result.reason.find(stage_name) != std::string::npos, "instance swap no-op reason must identify the sub-stage");
  return ok;
}
...
int main()
{
  ...
  ok &= verifyNoOp(detail_placer.runGlobalSwap(), "global swap");
  ok &= require(placer_db.get_revision() == revision_before, "global swap no-op must not advance PlacerDB revision");
  ok &= verifyNoOp(detail_placer.runVerticalSwap(), "vertical swap");
  ok &= require(placer_db.get_revision() == revision_before, "vertical swap no-op must not advance PlacerDB revision");
  ok &= require(wrapper->get_fixed()->get_coordi().get_x() == fixed_before.get_x()
                    && wrapper->get_fixed()->get_coordi().get_y() == fixed_before.get_y(),
                "instance swap must not move fixed instances");
  ok &= require(wrapper->get_movable()->get_coordi().get_x() == movable_before.get_x()
                ...
                "no-op instance swap must preserve movable coordinates");
