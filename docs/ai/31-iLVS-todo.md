...
- M0 已落地独立 graph JSON API/CLI 和 hand-written microcases；尚未落地 Verilog/iDB/GDS extraction。
...
- [ ] ReferenceLoader 支持 gate-level Verilog canonical cell/pin/net graph。
- [ ] LayoutConnectivityExtractor 从 iDB routing/pin/via/shape 生成 extracted graph。
- [ ] tech layer-purpose / via mapping 缺失必须 unsupported。
- [ ] 输出 graph counts、coverage、unmatched shape/pin/net。
  - [X] M0 graph JSON summary 已输出 graph counts / coverage；unmatched shape/pin/net 等 iDB extraction 字段未完成。

## P0 - matcher and diff reporter

- [ ] 实现 color refinement / partition matching / bounded backtracking。
  - [X] M0 已实现 deterministic signature partition matching；bounded backtracking 未完成。
...
- [ ] iDB extraction microcases：shape overlap、via connect、pin attach、unsupported layer。
...
|---|---|---|
| `src/operation/iLVS/` | M0 skeleton 已建 | 继续接 Verilog reference loader、iDB layout extractor |
| `reference loader` | 规划项 | canonical graph、provenance 独立 |
| `layout extractor` | 规划项 | iDB/GDS connectivity、coverage manifest |
| `matcher/diff reporter` | M0 signature partition + diff JSON | 继续补 bounded backtracking、最小不匹配子图 |
| `microcases` | M0 graph microcases 已覆盖 | 继续补 iDB extraction 和注入端到端 |
