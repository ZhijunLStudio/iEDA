...
| Pri | Leaf 文件 | 当前证据 / 风险 | 文件级任务 | DoD / focused test |
|---|---|---|---|---|
| P0 | `source/module/global_placer/electrostatic_placer/NesterovPlace.hh`, `NesterovPlace.cc` | 已有 typed outcome/result/iteration record；首次 invalid metric 会失败，GP failure 会阻断 flow 且失败 transaction 回滚；iteration exhibit 已接入 stage artifact；synthetic 和 JSON replay 仍缺 | 继续统一 divergence、invalid metric、overflow miss 的质量策略；随机扰动受 seed 控制；输出稳定 iteration exhibit，并保持 changed instance count 与 iteration count 分离 | `ipl_run_gp_result_test` 真实 `gcd_sky130_a` fixture 已断言 exhibit schema、严格递增 iter、finite 数值和 artifact 字段；仍需 synthetic 2/10/100 cell、NaN/death test、拥塞 A/B 和 JSON replay |
...
| P1 | `source/module/evaluator/wirelength/WAWirelengthGradient.hh`, `WAWirelengthGradient.cc` | WA 梯度可能受 gamma/指数溢出与高 fanout影响 | 使用稳定 log-sum-exp/截断策略并暴露 gamma；检查梯度和为零、finite；net weight 更新带 revision | finite-difference、极小 gamma、重合 pins、10k fanout；与 HPWL 趋势一致 |
...
- 已通过：本轮 12 项 iPL focused gate 全部通过，包含状态/结果传播、失败注入、真实 GP/MacroPlacer artifact、配置、HPWL、NFS、LayoutChecker、PlacerDB transaction、writeback、STA 和四个 DP 算子。
...
- 下一批必须补：Python BUILD_PYTHON=ON 命令级回归；APITest 进程级失败注入与全阶段 artifact schema golden；InstanceSwap accepted move/delta oracle；LocalReorder AES 上有益窗口或明确 no-op 门槛；wrapper malformed iDB 与中途 update/create rollback；Nesterov synthetic divergence、evaluator 数值 oracle 和至少一组默认策略 QoR A/B。
