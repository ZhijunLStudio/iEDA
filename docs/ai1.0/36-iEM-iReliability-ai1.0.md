...
## 12. Qualification、失效与 CI 细化

每个 `StressObservation` 的 qualification key 为 tech limit/model、geometry generation、current/activity scenario、temperature field、mission profile 和 tool hash。route/PDN/power/activity/temperature 任一改变都使对应 observation stale；增宽/加 via 后必须检查 whole component current redistribution，不能只更新修改 segment。

首批 gate 分开 `em.pg_wire`、`em.pg_via`、`em.signal_wire`、`em.signal_via`，每项有 checked/unsupported object 数。Limit 缺失时只有 stress metric，不产生 margin/pass。

CI：`REL-T01` wire/via/parallel mesh 解析电流，`REL-T02` width/cut/current/temp 单调，`REL-T03` average/RMS/peak/direction，`REL-T04` missing limit/activity/temp，`REL-T05` KCL/residual upstream fail，`REL-T06` current redistribution，`REL-T07` incremental/full，`REL-T08` mission model domain，`REL-T09` proposal secondary DRC/timing，`REL-T10` F2/F4 Recall@K。

PR：`REL-0 schema/TechLimit adapter` -> `REL-1 PG wire/via static` -> `REL-2 signal activity stress` -> `REL-3 temperature/mission plugin` -> `REL-4 proposal portfolio` -> `REL-5 F4 qualification`。
