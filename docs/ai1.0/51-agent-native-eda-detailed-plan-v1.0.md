...
| ID | 需求 | 优先级 | 系统级验收 |
|---|---|---:|---|
| FR-AN-01 | 任意工具调用绑定 immutable snapshot 与五类 context refs | P0 | 缺任一 required ref 时结构化拒绝，不读取 ambient current design |
| FR-AN-02 | capability discovery 返回 facet、domain、fidelity、权限、成本和 qualification | P0 | Planner 无法调用未发布/已撤销能力 |
| FR-AN-03 | inspect/diagnose/propose/apply/verify 分面使用 typed contract | P0 | proposal、delta、metric、certificate 不可互相冒充 |
| FR-AN-04 | 写操作只在 branch transaction 中执行 | P0 | 失败、取消、worker crash 后 main head hash 不变 |
| FR-AN-05 | 每个 delta 返回 declared/actual scope、DirtySet、inverse 与 invalidation | P0 | 越 scope 或 unknown dirty 不能进入 commit |
| FR-AN-06 | partial、unsupported、timeout、infeasible 和 validation failure 可区分 | P0 | 20 类故障注入均不被序列化为 success |
| FR-AN-07 | Runtime 按 policy 和 current certificate bundle 原子 commit | P0 | 缺 claim、coverage、artifact 或 stale ref 时零 commit 路径 |
| FR-AN-08 | F0-F4 路由由 domain qualification、风险和预算决定 | P0 | F0/F1 不可支持 F3/F4 claim，near-gate/OOD 必须升级或 unknown |
| FR-AN-09 | 候选用 hard gate + Pareto + 显式 tie-break 选择 | P0 | 任一硬门禁不可被 PPA 总分抵消 |
| FR-AN-10 | 每次实验生成可重放 evidence/decision/trajectory record | P0 | 从 CAS 和 journal 重建随机抽样决策 |
| FR-AN-11 | 支持 cancel、预算、checkpoint、resume 和有界恢复 | P1 | 无无限重试；cancel 后无孤儿 worker/lease |
| FR-AN-12 | 支持 branch-and-select 的并发候选探索 | P1 | 候选隔离、稳定 tie-break、head CAS 冲突可解释 |
| FR-AN-13 | 外部和生成工具通过 quarantine/isolated worker 接入 | P1 | 无任意 shell、主状态写入、secret/tenant 越权 |
| FR-AN-14 | 数据、模型、经验与 oracle 均有适用域和 lineage | P1 | family/tenant/PDK holdout 泄漏构造例全部拦截 |
| FR-AN-15 | capability maturity 由机器 evidence 晋级和撤销 | P1 | 文档、目录、命令存在不能提高 maturity |

### 23.2 非功能需求

| ID | 要求 | 测量方法 |
|---|---|---|
| NFR-AN-01 | 结果身份内容寻址；schema/context/tool/model/data/policy 变化显式换 ref | canonical hash 与 mutation test |
| NFR-AN-02 | 确定性路径可 bit/semantic replay；非确定性保存 seed、候选全集和统计 | replay corpus |
| NFR-AN-03 | 单位、对象 ID、scope、coverage、status 不以裸字符串跨边界 | schema conformance |
| NFR-AN-04 | legacy singleton 未证明线程安全前进 worker/session 隔离 | race/failure injection |
| NFR-AN-05 | tenant、license、retention 沿 artifact/cache/dataset/model/certificate 传播 | lineage/ACL property test |
| NFR-AN-06 | 资源上限覆盖 CPU/RSS/GPU/disk/output/license/wall/tool/model calls | quota adversarial test |
| NFR-AN-07 | 每个 production capability 有 owner、SLO、rollback、revocation 和 runbook | Registry readiness check |
| NFR-AN-08 | 性能只在冻结 protocol 上报告 median/MAD、尾延迟和 correctness | 42 Perf benchmark |
| NFR-AN-09 | schema minor/major 演化有兼容矩阵和 migration report | old/new fixture suite |
| NFR-AN-10 | degraded 模式不降低 correctness policy | dependency outage matrix |

所有模块文档必须把适用需求映射到 API、测试 ID 和 PR；不适用项要说明边界。系统验收按最弱依赖计算，不能以某个模块的局部 PASS 代替闭环完成。

## 24. 总契约：一次调用的类型闭包

### 24.1 `InvocationContext`

```yaml
invocation_id: uuid
actor: {tenant: acme, subject: agent-17, role: optimizer}
capability: {name: timing.top_paths, version: 1.2.0, qualification: sha256:...}
context:
  snapshot_ref: sha256:...
  intent_ref: sha256:...
  ...
  policy_ref: sha256:...
scope: {kind: object_set, ref: cas:scope/...}
fidelity: {requested: AUTO, minimum: F1, maximum: F3}
budget_ref: cas:budget/reservation
deadline: 2026-07-23T18:00:00+08:00
seed: 17
```

Capability manifest 决定哪些 ref 对某 facet 必填；Gateway 做 schema/auth，Runtime 再做 qualification/context/permission/head 检查。下游工具只接受 resolved immutable refs，不接受调用方提供的任意工作目录或隐式全局配置。

### 24.2 产物关系

```text
Observation   = read-only fact + coverage + provenance
Diagnosis     = evidence graph + hypotheses; no design write
Proposal      = intended typed action + declared scope + preconditions
TypedDelta    = materialized operation + inverse + actual touched/dirty
MetricRecord  = measured QoR; never a correctness proof
Certificate   = qualified validator claim; never a ranking score
Decision      = considered portfolio + policy + selected/rejected reasons
CommitRecord  = CAS head transition + current bundle + audit actor
```

每一类型有唯一 owner 文档；consumer 只能引用，不能复制字段后私自演化。`Proposal -> TypedDelta` 必须经过 Runtime/adapter materialization；`Metric -> Certificate` 没有隐式转换；`Certificate -> Commit` 还需 policy、head 和权限复核。
...
StageResult<T>
  ...
  context + capability/qualification refs
  completed_scope + remaining_scope + coverage
  value_ref<T> only when status permits
  diagnostics[] {code, category, retryable, evidence_refs}
  artifacts/provenance/resource/checkpoint refs
  invalidation events emitted
```

`status` 与 domain payload 分离：solver 可在 TIMEOUT 中返回有界 incumbent，parser 可在 PARTIAL 中返回已解析字段，但 consumer 必须按 manifest/policy 判断能否使用。message 只供人读，控制流只依据 enum 和 typed fields。

## 25. 系统一致性、事务与失效协议

### 25.1 一致性边界

- snapshot、context、manifest、policy 和 certificate 都是 immutable value object；
- experiment/branch/job/plan 是 event-sourced mutable aggregate，状态只按注册状态机推进；
- CAS 保存大 artifact，transaction catalog 保存 ref 与 head，event journal 保存顺序和 actor；
- branch apply 在 scope lease 下执行，commit 对 base head compare-and-swap；
- cache 是可丢派生状态，不能成为 snapshot、intent、tech 或证书真值；
- Registry revoke、context supersede、artifact tombstone 通过 outbox event 传播，consumer 在读时仍必须做 freshness check。

### 25.2 Commit protocol

```text
freeze candidate snapshot/context/capability set
  -> expand TypedDelta invalidation closure
  -> execute qualified validators on frozen subject
  -> issue current complete certificate bundle
  -> evaluate hard gates/Pareto/decision policy
  -> recheck actor permission, budget, base head and revocations
  -> atomic CAS(head: base -> candidate) + append CommitRecord
  -> publish invalidation/trajectory events
```

CAS 失败返回 `CONFLICT`，保留 branch 和证据；首版不自动 rebase/merge。CommitRecord 持久化失败时 head 不更新，event publish 失败由 transactional outbox 重试，不能产生“已提交但无审计记录”的状态。

### 25.3 失效传播

| 变化 | 最小失效对象 |
|---|---|
| DesignDelta | touched/dirty domain 的 metric、model feature、validator claim |
| Intent/Scenario | timing/power/clock/optimization baseline 与相关比较 |
| TechContext | mapping/RC/library/rule/physical qualification 与证书 |
| tool/parser/model | 其 result/cache/qualification；历史 evidence 保留 |
| policy | bundle/gate/decision，不必重跑不受影响的 raw metric |
| artifact 删除/retention | 依赖对象变 `UNVERIFIABLE`，不删除历史 ID |

失效函数只能保守扩大；无法计算 exact scope 时 full invalidation 或 `UNKNOWN`。任何“缓存仍有值”都不能覆盖 invalidation event。

## 26. 参考实现落点与依赖方向

```text
src/contracts/
  context/{object_id,snapshot_ref,intent_ref,tech_context_ref}.hh
  capability/{manifest,invocation,stage_result,error,coverage}.hh
  state/{scope,typed_delta,dirty_set,invalidation_set}.hh
  validation/{claim,certificate,bundle,policy}.hh
src/platform/design_state/            # snapshot catalog/CAS/transaction journal
src/platform/flow/tool_flow/          # deterministic DAG/stage adapter
src/platform/agent_runtime/            # experiment/branch/budget/commit/outbox
src/interface/agent/                   # transport-neutral gateway service
src/interface/mcp-iEDA/                # MCP adapter, no domain logic
src/evaluation/                        # MetricRecord/compare/gate/evidence
src/intent/ src/technology/ src/verification/
src/planner/ src/observer/ src/knowledge/experience/
src/operation/<tool>/agent/            # inspect/propose/apply/validator adapters
benchmarks/conformance/                # shared schema/state/security fixtures
```

建议构建目标：`ieda_agent_contracts` 仅依赖 STL/schema runtime；`ieda_design_state` 依赖 contracts 和 iDB adapter；`ieda_agent_runtime` 依赖 contracts/platform service interface，不链接领域 singleton；各 `*_agent_adapter` 依赖自己的 kernel 和 contracts；transport、Planner、Evaluation 只通过 service interface 使用 adapter。

首批不得做大目录搬迁。每个 PR 先增加 adapter/contract test，再逐步让旧 Tcl/API 调用经过同一 facade；旧消费者数值锁回归稳定后才删除旁路。

## 27. 系统测试、Benchmark 与 Release 资格

| Suite | 范围 | 关键失败判定 |
|---|---|---|
| AN-C01 Schema | context/request/result/unknown field/version/unit | 宽松吞 semantic field |
| AN-C02 State | snapshot/apply/rollback/replay/crash/head CAS | 主状态污染、hash 不可复现 |
| AN-C03 Scope | declared/actual/frozen/dirty/invalidation | scope 外任何未批准变化 |
| AN-C04 Failure | timeout/OOM/cancel/worker loss/artifact corrupt | partial/failure 被记 success |
| AN-C05 Validation | claim/coverage/stale/conflict/bundle | incomplete bundle 可 commit |
| AN-C06 Fidelity | F0-F4/OOD/near-gate/fallback | 低精度越级或 silent downgrade |
| AN-C07 Security | tenant/path/command/secret/network/package | 任一越权读取、写入或执行 |
| AN-C08 Data | lineage/family split/retention/tombstone | holdout/tenant 泄漏 |
| AN-C09 Replay | deterministic trace/non-deterministic evidence | 无法重建 decision/commit |
| AN-C10 Workflow | Timing/Route/PDN closed loops | 领域工具局部成功但闭环假提交 |

Release 从 `contract -> shadow -> qualified -> active` 逐级推进。每一级 evidence pack 固定源码、构建、schema、数据、模型、工具、PDK、policy 和 benchmark protocol hash。发现 invalid commit、tenant escape 或证书漏失效时，相关 capability 自动 suspended，回溯相同 qualification 的 active bundle，并以最小反例恢复。
...
## 附录 C：Timing ECO 参考执行序列

首个闭环必须先有一条不依赖 LLM 的 reference path。规则 Planner、人工请求和模型 Planner 都只能产生同一种 typed plan：

```text
1. Gateway authenticate + resolve CapabilitySnapshot
2. Runtime create Experiment(base SnapshotRef, IntentRef, ScenarioSetRef,
   TechContextRef, ValidationPolicyRef, BudgetRef)
3. Observer summarize + iSTA top_paths/path_explain
4. Planner enumerate typed ResizeInst/SwapVt/InsertBuffer proposals
5. Runtime validate permission/scope/budget and fork candidate branches
6. iDB apply TypedDelta in transaction; record touched/dirty/inverse sets
7. iPL legalize affected region when required
8. iRCX update dirty nets; iSTA update dirty timing cones
9. Evaluation normalize metrics; Router escalates near-gate/OOD candidates
10. Verification Hub expand invalidation closure and issue certificates
11. Selector keep verified Pareto candidates; Planner stop/revise
12. Runtime commit exactly one current complete bundle or rollback all branches
```

步骤 5、6、10、12 是确定性边界，Planner 不能跳过或重排。步骤 7-9 可以因 action/policy 不同形成 DAG，但每条数据边必须引用内容寻址 artifact，禁止通过进程内隐式全局状态传值。

### C.1 一次候选的持久化记录

```text
ExperimentRecord
  experiment/plan/revision/candidate IDs
  actor + capability snapshot
  base/branch/result snapshot refs
  intent/scenario/tech/policy/budget refs
  proposal + typed delta + scope/touched/dirty/inverse
  stage results + metrics + uncertainty/OOD
  validation plan/certificates/bundle
  decision/disposition/stop reason
  source/build/model/data/oracle/artifact hashes
  resource/license/latency events
```

状态变化先持久化 record/event，再对外发布完成状态。任何 `SUCCESS` 都必须有 result ref；任何 commit 都必须有 current bundle；任何失败都必须保留已完成 stage 和原始 failure cause。自然语言摘要是派生 artifact，不参与事务身份。

### C.2 并发与确定性点

- branch 从同一 immutable base 创建，apply 只能持有自己 scope lease；
- capability snapshot 在 plan 开始时固定，执行前再校验 retired/revoked；
- nondeterministic model/solver 保存 seed、request/response hash 和候选全集；
- Pareto 选择使用版本化 tie-breaker，不能依赖 map iteration 或完成顺序；
- commit compare-and-swap base head；head 已变化时返回 conflict，不自动 merge；
- cancel 后等待 worker/descendant 终止和 artifact 封存，再把 branch 标为 terminal。

## 附录 D：错误恢复矩阵

| 故障 | 状态 | 自动恢复上限 | 最终行为 |
|---|---|---:|---|
| capability retired/version drift | `STALE_CAPABILITY` | 重新解析 1 次，且 plan 未执行写节点 | 重新验证 plan 或停止 |
| worker crash/transport loss | `WORKER_LOST` | 幂等只读节点 1 次 | 写节点查 transaction journal 后 rollback/人工恢复 |
| low-fidelity OOD/near gate | `NEEDS_ESCALATION` | policy 定义 F2/F3/F4 | 无预算则保留 UNKNOWN，不提交 |
| local legalize/RC/STA partial | `PARTIAL` | 扩 scope 或 full 1 次 | 仍 partial 则候选拒绝 |
| external license unavailable | `RESOURCE_UNAVAILABLE` | 受预算的 backoff | 不能改写为物理 FAIL/PASS |
| certificate stale/coverage 缺失 | `INCOMPLETE` | 重新计划 required claim | 无 validator 则 UNSUPPORTED |
| commit head conflict | `CONFLICT` | 0 次自动 merge | 保留候选，基于新 head 重跑 |
| budget/user cancel | `CANCELLED` | 0 | 返回已验证 incumbent 与 residual risk |

恢复动作本身是 PlanNode/Event，消耗预算并可重放。禁止 catch-all 后从头无限重跑，也禁止因昂贵 validator 不可用而降低 required policy。

## 附录 E：Schema 演化与兼容策略

跨模块对象采用 major/minor schema version 和严格 reader：

- unknown major、unknown semantic field、unknown enum value 默认拒绝；
- minor 版本只允许新增明确声明 non-semantic/optional 的字段；
- 单位、ID、scope、coverage、status、permission 和 invalidation 都是 semantic；
- writer 不覆盖旧 artifact，migration 产生新 ref 和 migration report；
- capability manifest 声明接受的 request/response schema range；Gateway 在调用前协商；
- replay 使用原 schema/parser/tool；需要升级时同时保存 before/after semantic diff。

统一错误 envelope 至少含 `code/category/retryable/stage/message/evidence_refs/partial_outputs`。message 不参与控制流；Runtime 只根据枚举 code、manifest recovery policy 和预算决定恢复。

## 附录 F：发布资格与首批 issue 关闭条件

一个 Timing ECO action 从开发到可提交分五级：

| 级别 | 能力 | 关闭条件 |
|---|---|---|
| R0 Inspect | 查询/解释 | snapshot/context/coverage 完整，失败非 success |
| R1 Propose | 生成 typed proposal | scope/unsupported/预算可校验，无 DB side effect |
| R2 Shadow Apply | 隔离 branch apply/rollback | touched/dirty/inverse、crash recovery、主状态零污染 |
| R3 Controlled Commit | policy 下提交 | required bundle complete，500 branch invalid commit 为 0 |
| R4 Signoff assist | 高精度证据/审批 | external protocol、tenant/license、人工 signoff 边界明确 |

每个 issue 必须同时链接 owner contract、测试 ID、benchmark case 和 failure injection。PR 合并不自动提高 capability maturity；Registry 只有在冻结 evidence pack 通过后更新 qualification。首批 action 中任何一个出现 silent partial、scope 越界、constraint drift 或 certificate 漏失效，所有写 action 停留在 R2，优先修共同底座。
