...
## 12. Mesh identity、守恒与增量边界

`ThermalResultKey` 包含 Assembly/geometry、material/boundary、power map/scenario、mesh definition、solver/tolerance 和 tool hash。Mesh 用 origin/transform/cell dimensions/stable region mapping 标识；不同 mesh 的温度需通过声明插值 operator 对齐，不能逐 index 比较。

增量首版只允许 power map 值更新而 geometry/material/boundary 不变；局部热求解必须提供边界误差估计。宏移动、封装边界或材料变化默认 full rebuild。Energy gate 固定检查 injected power、boundary flux 和 storage term（transient）；不平衡结果为 numerical failure。

## 13. CI 编号与 PR

`TH-T01` 1D slab/均匀源解析解，`TH-T02` 多材料 interface，`TH-T03` energy balance，`TH-T04` mesh refinement，`TH-T05` power/ambient/k 单调，`TH-T06` coordinate/unit mapping，`TH-T07` missing boundary/material，`TH-T08` coupled convergence/runaway，`TH-T09` incremental/full，`TH-T10` external oracle/selection regret。

PR：`TH-0 schema/boundary/power adapter` -> `TH-1 steady grid+gold` -> `TH-2 multilayer/mesh convergence` -> `TH-3 coupled fixed point` -> `TH-4 proposal/evaluation` -> `TH-5 transient/3D/F4`。
