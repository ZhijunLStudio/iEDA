...
本计划不设一个可被权重操纵的“AI EDA 总分”。项目经营看四组指标，并以 correctness 为前置门禁：

| 层级 | 核心问题 | 首批指标 | 数据来源 |
|---|---|---|---|
| Correctness | 是否提交了不该提交的设计 | invalid commit、证书漏失效、rollback/replay mismatch、hard-gate false negative | 43/49 Runtime 与证书日志 |
| Closure | 是否更快得到可行改进 | time-to-first-feasible、WNS/TNS、hold/DRV、accepted action rate | Timing Lab frozen benchmark |
| Decision | 同等验证预算下是否选得更好 | Recall@K、selection regret、calibration/OOD、F3/F4 升级率 | 12/47 evaluation/oracle |
| Economics | 是否降低真实闭环成本 | CPU/RSS/wall、license minutes、失败重试、cache hit correctness、人工复核时间 | 40/42 Platform/Perf |
| Leverage | 底座是否能复用到第二工具 | 新 adapter lead time、共用 schema 比例、escaped defect、重复实现数 | 46 Registry 与架构审计 |

所有改善必须相对冻结 baseline、相同 design-family/PDK split、相同 hard gate 和预算报告置信区间。只报 winner、不报失败候选，或只报平均值不报 near-gate/尾部风险，均不能进入里程碑评审。

M0 的任务不是证明 Agent 有效，而是冻结测量系统：基线版本、oracle protocol、数据 split、失败分类和运行成本。M1-M4 才逐步验证 closure、generalization 和 leverage。

## 12. Program Gate 与投资闸门

每个新能力依次通过以下 gate；日期是计划，不替代机器退出条件：

| Gate | 需要证明 | 未通过时允许做什么 | 禁止做什么 |
|---|---|---|---|
| PG0 Fact | 当前资产、owner、consumer、oracle 清楚 | D0 研究、adapter spike | 宣称已有生产能力 |
| PG1 Contract | schema、状态、权限、失败、coverage 冻结 | read-only/shadow | 主状态写入 |
| PG2 Closed Loop | 一个真实 action 有 branch/rollback/certificate | 限定 benchmark R2 | 自动 commit、扩工具面 |
| PG3 Generalization | held-out family/PDK、OOD/fallback 达标 | 小流量、显式审批 | 关闭高精度回落 |
| PG4 Product | SLO、隔离、升级/回滚、成本可运营 | 受 policy 的生产调用 | 越过 signoff 边界承诺 |

跨 gate 的唯一依据是 evidence pack。演示、代码量、模型参数量、API 数量和 compile pass 不能替代退出证据。若发现 invalid commit、数据泄漏、静默 partial 或增量/full 系统性不一致，相关 workstream 回退到 PG1/PG2 并暂停新增 action class。

资源优先级固定为：先维护测量/oracle 与状态验证关键路径，再做 Timing Lab，最后才增加模型和工具生成范围。GPU、商业 license 或新团队投入要绑定一个可杀假说和停止条件，不能以“未来可能需要”永久占用。

## 13. 研发运营与决策节奏

- 每周：合同变更、invalid/partial、增量/full diff 和 top failure class；
- 每双周：Timing Lab frozen cases 与预算等价 A/B；
- 每月：held-out family/PDK、商业 oracle protocol、成本和安全审计；
- 每里程碑：冻结 source/build/model/data/policy hashes，发布 evidence pack 与未解决风险；
- 每季度：重新决定第二工具、部署形态、模型/生成投入，未达假说的路线降级或停止。

决策记录至少写明：问题、备选方案、前置事实、选择、可逆性、owner、复审日期和触发回滚的观测。重要 schema/权限/signoff 边界的改变必须同步到 `51`-`57` owner 文档和 `52` 实施索引。

## 14. 从战略到代码的追踪链

```text
strategic hypothesis (50)
  -> architecture invariant / product slice (51, 53)
  -> module owner and contract (10-49, 54-57)
  -> issue / PR / CI test (52)
  -> frozen evidence pack (12, 42, 47, 49)
  -> milestone decision and rollback rule
```

每个里程碑必须能沿此链回答“哪项代码和哪组证据支持哪条战略判断”。无法追踪的研究结果可以保留为实验，但不能用于提高 maturity、取消 fallback 或对外承诺。

**版本历史**：
- v1.2（2026-07-23）：增加战略指标树、Program Gate、研发决策节奏和从假说到证据/代码的追踪链。
